...
function exportToExcel(data: PPCRow[], fileName = "PPC_Export"): void {
  ...
  const header = ["Produkt", "Typ textu", "C.", "Text", "Znaku", "Limit", "Status"];
...
function getStatus(len: number, max: number): string {
  if (len === 0) return "";
  if (len > max) return "Pres limit";
  if (len >= max - 4) return "Na hrane";
  return "OK";
}

const TABS = [
  { key: "settings", label: "Nastaveni" },
  { key: "checklist", label: "Checklist" },
  { key: "generate", label: "Generator textu" },
  { key: "results", label: "Vygenerovane texty" },
...
export default function CampaignManager() {
  ...
  const refreshCampaign = useCallback(async (idx: number, allCampaigns: Campaign[]) => {
    ...
    if (!c?.id) return;
    const { data, error } = await supabase.from("campaigns").select("*").eq("id", c.id).single();
    if (!error && data) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (error) {
        ...
        setCampaigns([defaultCampaign("Nova kampan")]);
      } else if (data && data.length > 0) {
        setCampaigns(data.map(dbToCampaign));
      } else {
        const def = defaultCampaign("Nova kampan");
        const { data: inserted, error: insertErr } = await supabase
          .from("campaigns")
          .insert({ name: def.name, products: def.products, checklist: def.checklist, google_texts: def.googleTexts, sklik_texts: def.sklikTexts, meta_texts: def.metaTexts })
          .select().single();
  ...
  const saveCampaign = useCallback(async (camp: Campaign) => {
    ...
    await supabase.from("campaigns").update({
      name: camp.name, products: camp.products, checklist: camp.checklist,
      google_texts: camp.googleTexts, sklik_texts: camp.sklikTexts, meta_texts: camp.metaTexts,
  ...
  const addProduct = (name: string) => update(c => { if (!c.products.includes(name)) c.products.push(name); });

  const removeProduct = (name: string) => update(c => {
  ...
  const setCustomText = (product: string, typeId: string, idx: number, val: string) => update(c => {
  ...
  const setChecklistStatus = (product: string, itemLabel: string, val: string) => update(c => {
  ...
  const overallPct = () => camp.products.length === 0
    ? 0
  ...
  const generateTexts = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("generate-fan-texts", {
        body: {
          product: genBrief.product, usp: genBrief.usp, cta: genBrief.cta,
          audience: genBrief.audience, tone: settings.tone, clientName: settings.clientName,
          textTypes: settings.textTypes.map(t => ({ id: t.id, label: t.label, count: t.count, maxLength: t.maxLength })),
        },
      });
      ...
      toast.success("Texty pro " + p + " vygenerovany!");
      setActiveTab("results");
    } catch (e: any) {
      console.error("Generation error:", e);
      toast.error(e.message || "Chyba pri generovani textu.");
    }
    setGenerating(false);
  };
    ...
    camp.products.forEach(p => {
      ...
      settings.textTypes.forEach(t => {
        ...
        texts.forEach((text, i) => {
          if (!text) return;
          rows.push({ platforma: t.channel || "Ostatni", produkt: p, typTextu: t.label, cislo: i + 1, text, znaku: text.length, limit: t.maxLength, status: getStatus(text.length, t.maxLength) });
        });
      });
    });
    if (rows.length === 0) { toast.error("Zadne texty k exportu."); return; }
    exportToExcel(rows, camp.name.replace(/[^a-zA-Z0-9_-]/g, "_"));
    toast.success("Export dokoncen!");
  };
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-muted-foreground text-lg">Nacitam kampane...</div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-background">
      <div className="bg-fan-navy text-primary-foreground px-6 py-4 flex items-center justify-between no-print">
        <div>
          ...
          <p className="text-xs text-primary-foreground/60 mt-0.5">
            {settings.clientName ? "Klient: " + settings.clientName : "Univerzalni generator PPC textu"}
          </p>
        </div>
        <div className="flex gap-2 items-center flex-wrap">
          <button
            onClick={async () => {
              const name = prompt("Zadejte nazev noveho projektu:", "Novy projekt");
              ...
              const { data: inserted, error } = await supabase
                ...
                .insert({ name: def.name, products: def.products, checklist: def.checklist, google_texts: def.googleTexts, sklik_texts: def.sklikTexts, meta_texts: def.metaTexts })
                .select().single();
              if (error || !inserted) { toast.error("Chyba pri vytvareni projektu"); return; }
              ...
              toast.success("Projekt " + name.trim() + " vytvoren!");
            }}
          ...
          {campaigns.map((c, i) => (
            <div
              key={c.id || i}
              className={"group relative flex items-center gap-1 px-3 py-1.5 rounded-md text-sm border-none cursor-pointer transition-colors " + (i === activeIdx ? "bg-primary text-primary-foreground font-bold" : "bg-fan-navy-light text-primary-foreground/80 hover:bg-primary/60")}
            ...
            >
              ...
              {campaigns.length > 1 && editingTabIdx !== i && (
                <button
                  onClick={async (e) => {
                    e.stopPropagation();
                    if (!confirm("Opravdu smazat kampan " + c.name + "?")) return;
                    if (!c.id) return;
                    const { error } = await supabase.from("campaigns").delete().eq("id", c.id);
                    if (error) { toast.error("Chyba pri mazani kampane"); return; }
                    setCampaigns(prev => prev.filter((_, idx) => idx !== i));
                    setActiveIdx(prev => Math.min(prev, campaigns.length - 2));
                    toast.success("Kampan smazana.");
                  }}
      ...
      <div className="bg-card border-b border-border px-6 py-2.5 flex items-center gap-6 no-print">
        <div className="flex-1">
          <div className="text-xs text-muted-foreground mb-1">{"Celkova dokoncenost: " + pct + "%"}</div>
          <div className="h-2 bg-muted rounded-full overflow-hidden">
            <div
              className={"h-full rounded-full transition-all duration-300 " + (pct === 100 ? "bg-status-done" : pct > 50 ? "bg-primary" : "bg-status-pending")}
              style={{ width: pct + "%" }}
            />
          </div>
        </div>
          ...
          {camp.products.map(p => {
            ...
            return (
              <div key={p} className="text-center">
                ...
                <div className={"text-base font-bold " + (pc === 100 ? "text-status-done" : pc > 50 ? "text-primary" : "text-status-pending")}>
                  {pc + "%"}
                </div>
              </div>
            );
      ...
      <div className="bg-card border-b border-border px-6 flex no-print">
        {TABS.map(t => (
          <button
            ...
            className={"bg-transparent border-none px-4 py-3 cursor-pointer text-sm transition-colors " + (activeTab === t.key ? "border-b-[3px] border-b-primary text-primary font-bold" : "border-b-[3px] border-b-transparent text-muted-foreground hover:text-foreground")}
          >
            {t.label}
          </button>
        ))}
      ...
      <div className="p-6">
        {activeTab === "settings" && (
          <SettingsTab settings={settings} setSettings={setSettings} onSaveDefault={() => { saveSettings(settings); toast.success("Nastaveni ulozeno."); }} />
        )}
        {activeTab === "checklist" && <ChecklistTab camp={camp} setChecklistStatus={setChecklistStatus} />}
        {activeTab === "generate" && (
          <GeneratorTab camp={camp} genBrief={genBrief} setGenBrief={setGenBrief} generating={generating} onGenerate={generateTexts} settings={settings} onAddProduct={addProduct} onRemoveProduct={removeProduct} />
        )}
        {activeTab === "results" && (
        ...
        {["google_pmax", "google_search", "sklik_search", "sklik_display", "meta", "linkedin", "bannery"].map(tabKey => {
          ...
          const filteredSettings = { ...settings, textTypes: settings.textTypes.filter(t => t.channel === channelName) };
          return (
            <div key={tabKey}>
